...
export async function before(m, { conn }) {
 ...
 //   let apii = await this.getFile(pp)
	if (m.messageStubType == 21) {
		await this.sendMessage(m.chat, { text: `${usuario}وضع اسم جديد للجروب:\n\n*${m.messageStubParameters[0]}*`, mentions: [m.sender] }, { quoted: fkontak }) 
	} else if (m.messageStubType == 22) {
		await this.sendMessage(m.chat, { text: `${usuario} وضع صورة جديده للجروب`, mentions: [m.sender] }, { quoted: fkontak }) 
	} else if (m.messageStubType == 24) {
		await this.sendMessage(m.chat, { text: `${usuario} وضع وصف جديد للجروب:\n\n${m.messageStubParameters[0]}`, mentions: [m.sender] }, { quoted: fkontak })
  ...
  /* } else if (m.messageStubType == 28) {
   conn.sendMessage(m.chat, { text: `${usuario} ELIMINO A @${m.messageStubParameters[0].split`@`[0]} 🫵`, mentions: [m.sender]}, { quoted: fkontak })  */
		...
		await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} *افـرح يــعم بـقـيت مـشرف*`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`] }, { quoted: fkontak })
	} else if (m.messageStubType == 30) {
		await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} *اكــيد كـنت مــتـخاذل*\n`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`] }, { quoted: fkontak })
