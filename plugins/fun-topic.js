...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (m.sender in conn.cartoon)
    throw "*انــتـظر لــم انــتـهي من الـــعـملــية الاولــى !*";
  let q = m.quoted ? m.quoted : m;
  let mime = (q.msg || q).mimetype || q.mediaType || "";
  if (!mime) throw `*ايــن الــصورة !*`;
  if (!/image\/(jpe?g|png)/.test(mime)) throw `file ${mime} not supported`;
  else conn.cartoon[m.sender] = true;
  m.reply("*جــاري تــحـويــل الــصورة...*");
  ...
  try {
    Cartoon(img).then(async (response) => {
      if (response.message == "success") {
        ...
        m.reply(
          "*لــم اســتــطــع تــحويــلـها !*"
        );
      }
    });
  } catch {
    m.reply("*فــشلــت الــعـملــية !*");
  } finally {
