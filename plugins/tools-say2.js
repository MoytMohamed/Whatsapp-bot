...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `*يــجب أن تــخـتر احــد الــتأثــيـرات الــقادمــة :*\n`;

    for (let i = 0, count = 0; count < 100 && i < voiceList.resultado.length; i++) {
      const entry = voiceList.resultado[i];
      if (entry.ID.length <= 20) {
        responseText += `*${usedPrefix + command} ${entry.ID} الــكلام*\n`;
        count++;
      }
    }
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `*هــذا الــصوت غــير مــوجـود !*\n*${usedPrefix + command} لــلـحــصول عــلى قــائــمة الــتـأثــيـرات !*` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `*نــسـيت تــرسل الــكلام !*\n\n*مـــثال : ${usedPrefix + command} ${efecto} الــكلام*`}, {quoted: m});
