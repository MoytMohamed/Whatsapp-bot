        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.sendButton(m.chat, `*لــيس لــديــك جــواهــر كــافــية لأســتـخدام هــذا الأمــر !*\n*اســتخــدم هــذا الامــر لـلــحصـول عــلى جــواهــر : ${usedPrefix}لــجواهــر*\n*لـلــعلــم أن الــجــواهــر تأتــي مـن تــحويــل 350 اكـس بــي الــى جــواهــر !*`, igfg, null, [['Buy', `${usedPrefix}todiamond`], ['Buy All', `${usedPrefix}todiamondall`]], m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`*اســخـدمــت ${+m.diamond} مــن الــجـواهــر 💎*`)
                }
                break
            }
        }
...
export async function deleteUpdate(message) {
    try {
        ...
        if (chat.delete)
            return 
            await this.reply(msg.chat,`*الرقم:* @${participant.split`@`[0]}`.trim(), msg, {
